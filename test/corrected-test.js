/**
 * Test suite for the GRIB2 decoder - corrected version
 *
 * Each test constructs a synthetic GRIB2 message byte-by-byte
 * based on the GRIB2 specification, then verifies decoding.
 *
 * GRIB2 message layout:
 *   [Section 0: 8 bytes (GRIB signature + edition + length)]
 *   [Section 1: header(4) + s1Len data]
 *   [Section 2: header(4) + s2Len data]
 *   [Section 3: header(4) + s3Len data]
 *   [Section 4: header(4) + s4Len data]
 *   [Section 5: header(4) + s5Len data]
 *   [Section 6: header(4) + s6Len data] (optional)
 *   [Section 7: header(4) + s7dataLen data + 4 bytes "7777"]
 */
...
function buildMessage(packingType, numPoints, bitsPerValue, referenceValue,
    ...
    u32(buf, off + 4, numPoints); // Fixed: should be at offset 4 (not 5)
    u8(buf, off + 8, 0); // bitmap indicator (present) - Fixed: should be at offset 8 (not 4)
...
// Test 1: Simple packing - byte-aligned (8 bits per value)
{
    ...
    // coded = round(value / 10^1): [1, 2, -1, 10], decoded = [10, 20, -10, 100]
    assert(Math.abs(result.values[0] - 10.0) < 0.1, `values[0] ≈ 10.0 (actual: ${result.values[0]})`);
    assert(Math.abs(result.values[1] - 20.0) < 0.1, `values[1] ≈ 20.0 (actual: ${result.values[1]})`);
    assert(Math.abs(result.values[2] - (-10.0)) < 0.1, `values[2] ≈ -10.0 (actual: ${result.values[2]})`);
    assert(Math.abs(result.values[3] - 100.0) < 0.1, `values[3] ≈ 100.0 (actual: ${result.values[3]})`);
...
// Test 2: Simple packing - non-byte-aligned (4 bits per value)
{
    ...
    assert(result.values.length === numPoints, `values length = ${numPoints}, got ${result.values.length}`);
    for (let i = 0; i < numPoints; i++) {
        assert(Math.abs(result.values[i] - (i + 1)) < 0.01,
               `values[${i}] ≈ ${i + 1} (actual: ${result.values[i]})`);
    }

    console.log(`  values: [${result.values.join(', ')}]`);
}

// Test 3: Simple packing with reference value and scale factors
{
    ...
    // coded = round((value - 20) / 10): [1, 1, 1, 1]
    // decoded = [30, 30, 30, 30] (lossy round-trip)
    // The decoder should decode exactly what was coded
    assert(Math.abs(result.values[0] - 30.0) < 0.1, `values[0] ≈ 30.0 (actual: ${result.values[0]})`);
    assert(Math.abs(result.values[1] - 30.0) < 0.1, `values[1] ≈ 30.0 (actual: ${result.values[1]})`);
    assert(Math.abs(result.values[2] - 30.0) < 0.1, `values[2] ≈ 30.0 (actual: ${result.values[2]})`);
    assert(Math.abs(result.values[3] - 30.0) < 0.1, `values[3] ≈ 30.0 (actual: ${result.values[3]})`);

    console.log(`  values: [${result.values.join(', ')}]`);
}

// Test 4: Simple packing with binary scale factor
{
    ...
    assert(Math.abs(result.values[0] - 2.0) < 0.01, `values[0] ≈ 2.0 (actual: ${result.values[0]})`);
    assert(Math.abs(result.values[1] - 3.0) < 0.01, `values[1] ≈ 3.0 (actual: ${result.values[1]})`);
    assert(Math.abs(result.values[2] - 4.0) < 0.01, `values[2] ≈ 4.0 (actual: ${result.values[2]})`);
    assert(Math.abs(result.values[3] - 5.0) < 0.01, `values[3] ≈ 5.0 (actual: ${result.values[3]})`);
...
// Test 5: Constant value packing (type 40)
{
    ...
    assert(result.values.length === numPoints, `values length = ${numPoints}, got ${result.values.length}`);
    for (let i = 0; i < numPoints; i++) {
        assert(Math.abs(result.values[i] - 42.0) < 0.01, `values[${i}] ≈ 42.0 (actual: ${result.values[i]})`);
    }

    console.log(`  values: [${result.values.join(', ')}]`);
}

// Test 6: IEEE 754 packing (type 254)
{
    ...
    assert(result.values.length === numPoints, `values length = ${numPoints}, got ${result.values.length}`);
    assert(Math.abs(result.values[0] - 1.5) < 0.001, `values[0] ≈ 1.5 (actual: ${result.values[0]})`);
    assert(Math.abs(result.values[1] - (-3.14159)) < 0.001, `values[1] ≈ -3.14159 (actual: ${result.values[1]})`);
    assert(result.values[2] === 0.0, `values[2] = 0.0 (actual: ${result.values[2]})`);
    assert(Math.abs(result.values[3] - 100.0) < 0.001, `values[3] ≈ 100.0 (actual: ${result.values[3]})`);
...
// Test 7: All-missing values (type 255)
{
    ...
    assert(result.values.length === numPoints, `values length = ${numPoints}, got ${result.values.length}`);
    for (let i = 0; i < numPoints; i++) {
        assert(result.values[i] === -1e100, `values[${i}] = -1e100 (actual: ${result.values[i]})`);
    }

    console.log(`  values: [${result.values.join(', ')}]`);
}

// Test 8: Header parsing (no data decoding)
{
    ...
    console.log(`  dataOffset=${result.dataOffset}, dataLength=${result.dataLength}`);
}
...
// Test 9: Section walking
{
    ...
    console.log(`  sections: ${Object.keys(sections.sections).join(', ')}`);
    console.log(`  dataOffset=${sections.dataOffset}, dataLength=${sections.dataLength}`);
}

// Test 10: Larger dataset (100 points)
{
    console.log('\nTest 10: Larger dataset (100 points)');
    ...
    let maxError = 0;
    for (let i = 0; i < numPoints; i++) {
        const expected = Math.sin(i * 0.1) * 10 + 20;
        const error = Math.abs(result.values[i] - expected);
        maxError = Math.max(maxError, error);
        assert(error < 0.1, `values[${i}] error < 0.1 (error: ${error})`);
    }

    console.log(`  values length: ${result.values.length}`);
    console.log(`  max error: ${maxError}`);
    console.log(`  first 5: [${result.values.slice(0, 5).join(', ')}]`);
}

// Test 11: Grid with Ni x Nj
{
    console.log('\nTest 11: Grid with Ni x Nj (not just 1 row)');
    ...
    const buffer = buildMessage(0, numPoints, 8, 0, 0, 0, values, {
        latitudeOfFirstPoint: 60,
        longitudeOfFirstPoint: -180,
        latitudeOfLastPoint: 30,
        longitudeOfLastPoint: 180,
        Ni, Nj,
    });

    const result = decodeGRIB2(buffer);

    assert(result.grid.ni === Ni, `Ni = ${Ni}, got ${result.grid.ni}`);
    assert(result.grid.nj === Nj, `Nj = ${Nj}, got ${result.grid.nj}`);
    assert(result.grid.totalPoints === numPoints, `totalPoints = ${numPoints}, got ${result.grid.totalPoints}`);
    assert(result.values.length === numPoints, `values length = ${numPoints}, got ${result.values.length}`);

    console.log(`  Ni=${result.grid.ni}, Nj=${result.grid.nj}, total=${result.grid.totalPoints}`);
    console.log(`  values: [${result.values.join(', ')}]`);
}

// Test 12: Invalid signature detection
{
    console.log('\nTest 12: Invalid signature detection');

    const invalid = new ArrayBuffer(8);
    const view = new Uint8Array(invalid);
    view[0] = 0x41; // 'A' instead of 'G'
    view[1] = 0x42; // 'B' instead of 'R'
    view[2] = 0x43; // 'C' instead of 'I'
    view[3] = 0x44; // 'D' instead of 'B'
    view[4] = 2;
    view[5] = 0; view[6] = 0; view[7] = 8;
